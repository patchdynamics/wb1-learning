...
do
  RES=$(sbatch --job-name=p${ALG:0:3}${MODE:0:3}  --dependency=afterok:${RES##* } --array=0-$PEERS -p DGE --mail-type=FAIL --mail-user=shultzm@stanford.edu multicore/invoke.$MODE.sh $ALG $epsilon $FLAGS )
  RES=$(sbatch --job-name=c${ALG:0:3}${MODE:0:3} --dependency=afterok:${RES##* } -p DGE  multicore/combine.weights.sh   ../multicore/$MODE.$ALGl$FLAGS   $PEERS  $ALG)
done
